...
func (h *CaddyHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	if valid {
		// let upstream middleware (e.g. fastcgi and cgi) know about authenticated user; this replaces the request with a wrapped instance
		r = r.WithContext(context.WithValue(r.Context(), httpserver.RemoteUserCtxKey, userInfo.Sub))
